...
import { LayoutDashboard, Sliders, SlidersHorizontal, History, Settings, ChevronLeft, ChevronRight, LogOut } from 'lucide-react';
...
export function Sidebar({ currentTab, onChangeTab, data }: SidebarProps) {
  ...
  const tabs = [
    ...
    { id: 'operator', label: 'Operatör Paneli', icon: SlidersHorizontal },
    { id: 'history', label: 'Raporlar', icon: History },
  ];
      ...
      <div className={cn("p-2", !isCollapsed && "p-3")}>
        ...
        <nav className="space-y-1">
          {tabs.map((tab) => {
            ...
            const Icon = tab.icon;
            const isDisabled = isProcessActive && tab.id !== 'dashboard';
            
            return (
              <button
                key={tab.id}
                title={isCollapsed ? (isDisabled ? "Döngü Sırasında Kilitli" : tab.label) : undefined}
                disabled={isDisabled}
                ...
                className={cn(
                  ...
                  isDisabled 
                    ? "opacity-25 cursor-not-allowed hover:bg-[#151921] hover:text-gray-500" 
                    : "hover:bg-[#1C2029] hover:text-gray-250"
                )}
              >
                <Icon size={16} />
          ...
          <button 
             title={isCollapsed ? (isProcessActive ? "Sistem Aktif - Kilitli" : "Yapılandırma") : undefined}
             ...
             className={cn(
               ...
               isProcessActive 
                  ...
                  : "hover:bg-[#1C2029] hover:text-gray-200"
             )}
          >
            <Settings size={16} />
