...
import { Server, Trash2, Plus, Terminal, Send, Activity, Settings2, Link, RefreshCw, Lock } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden space-y-4">
      ...
      <div className="flex-1 overflow-hidden relative">
        {!data.isEngineerMode && activeTab !== 'TERMINAL' && (
          <div className="absolute inset-0 bg-[#0a0f18]/80 backdrop-blur-[2px] z-[50] flex flex-col items-center justify-center p-6 text-center">
             <div className="bg-[#151921] border border-amber-500/30 p-8 rounded-xl shadow-2xl max-w-sm">
                <div className="w-16 h-16 bg-amber-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-amber-500/20">
                   <Lock size={32} className="text-amber-500 animate-pulse" />
                </div>
                <h3 className="text-lg font-bold text-white mb-2 tracking-tight">Donanım Erişimi Kilitli</h3>
                <p className="text-gray-400 text-xs leading-relaxed mb-6">
                  Bağlantı ayarları, pin tanımlamaları ve donanım yapılandırması sadece <b>Mühendis Modunda</b> değiştirilebilir.
                </p>
                <div className="py-2 px-4 bg-amber-500/5 rounded border border-amber-500/10 text-[9px] text-amber-500 font-black tracking-widest uppercase">
                  YETKİLİ ERİŞİMİ GEREKLİ
                </div>
             </div>
          </div>
        )}
                ...
                <div key={sensor.id} className={cn(
                  ...
                  <div className="flex items-center space-x-3 mb-2">
                    <div className={cn(
                      ...
                      sensor.enabled 
                        ? (sensor.isTriggered ? "bg-amber-400 shadow-[0_0_12px_rgba(251,191,36,0.8)] scale-110" : "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]") 
                        : "bg-gray-600"
                    )} />
                    <input 
                       type="text" 
        ...
        <div className="h-full overflow-y-auto pr-2 pb-10">
          <div className="bg-[#151921] border border-[#2D333F] rounded p-4 flex flex-col">
            ...
            {/* Toplu Ayar Paneli */}
            <div className="bg-[#1C2029] border border-[#374151] rounded p-3 mb-4 flex items-center justify-between shadow-lg shadow-black/20">
              <div className="flex items-center space-x-6">
                 <div className="flex flex-col">
                    <label className="text-[9px] text-gray-500 font-bold mb-1 uppercase tracking-wider">Toplu Dolum Süresi (ms)</label>
                    <div className="flex items-center space-x-2">
                       <input 
                          type="number" 
                          id="bulk-fill-time"
                          placeholder="Örn: 3000"
                          defaultValue={data.valves[0]?.pulseDuration || 1000}
                          className="w-24 bg-[#0D1016] border border-[#374151] text-[11px] font-mono text-orange-400 rounded px-2 py-1 outline-none focus:border-orange-500/50"
                       />
                       <button 
                          onClick={() => {
                             const input = document.getElementById('bulk-fill-time') as HTMLInputElement;
                             const val = Number(input.value);
                             if (val > 0) {
                                data.valves.forEach(v => onUpdateValve?.(v.id, { pulseDuration: val }));
                             }
                          }}
                          className="bg-blue-600 hover:bg-blue-500 text-white text-[9px] font-bold px-3 py-1 rounded transition-all active:scale-95 shadow-lg shadow-blue-900/20"
                       >
                          TÜMÜNE UYGULA
                       </button>
                    </div>
                 ...
                 <div className="h-8 w-px bg-[#2D333F]" />
                 
                 <div className="flex flex-col">
                    <label className="text-[9px] text-gray-500 font-bold mb-1 uppercase tracking-wider">Aktif Valf Sayısı (Grup)</label>
                    <div className="flex items-center space-x-2">
                       <span className="text-emerald-400 font-black text-xs">{data.config.targetCount}</span>
                       <span className="text-[9px] text-gray-600 uppercase font-bold">ŞİŞE / DÖNGÜ</span>
                    </div>
                 </div>
              </div>
              
              <div className="hidden lg:block text-[9px] text-gray-500 max-w-[200px] italic leading-tight text-right">
                 * Toplu ayarlar tüm valf modüllerine anında iletilir. Bireysel düzeltmeleri aşağıdaki kartlardan yapabilirsiniz.
              </div>
            </div>
              ...
              {data.valves.map(valve => (
                <div key={valve.id} className={cn(
                  ...
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div>
                      <div className={cn("text-xs font-bold", valve.enabled ? "text-gray-200" : "text-gray-500")}>Valf Modülü #{valve.id}</div>
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <div className="flex flex-col w-16 group relative shrink-0">
                        <label className="text-[9px] text-gray-500 font-bold mb-1" title="Otomatik modda vananın açık kalacağı süre">Açık (ms)</label>
                        <input 
                          type="number" 
        ...
        {activeTab === 'TERMINAL' && (
          <div className="bg-[#151921] border border-[#2D333F] rounded p-4 flex flex-col h-full">
            ...
            <div className="flex-1 bg-[#050505] border border-[#1F2937] rounded flex flex-col h-0 relative">
               <div className="absolute inset-0 p-3 overflow-y-auto flex flex-col-reverse font-mono text-[10px] text-gray-400 space-y-1 space-y-reverse">
                  {data.terminalLogs?.map((log, i) => (
                     <div key={i} className={
                       log.includes('ERR') ? 'text-red-400' : 
                       log.includes('->') ? 'text-blue-400 border-l-2 border-blue-500 pl-2 ml-1' : 'text-gray-300'
                     }>
                       {log}
                     </div>
                  ))}
