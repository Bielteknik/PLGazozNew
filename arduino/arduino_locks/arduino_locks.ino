...
GateConfig gate1 = {2, 5, 8, 1000, 500, 200, 0};
GateConfig gate2 = {3, 6, 8, 1000, 500, 200, 0};
...
void loop() {
  if (Serial.available() > 0) {
    ...
    // FORMAT: CONF_G:ID:STEP:DIR:ENA:SPEED:ACCEL:OPEN:CLOSE
      ...
      if (id == 1) {
        ...
        // Not: Pinler değişirse objeyi yeniden başlatmak gerekebilir. 
        // Şimdilik hız ve limitleri güncelliyoruz.
        stepper1.setMaxSpeed(gate1.speed);
        stepper1.setAcceleration(gate1.accel);
        pinMode(gate1.enaPin, OUTPUT);
      } else if (id == 2) {
  ...
  // Eğer her iki motor da hedefe ulaştıysa, cızırtıyı kesmek için sürücüleri kapat (ENA -> HIGH)
