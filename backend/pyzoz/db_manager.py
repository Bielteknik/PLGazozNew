...
class DatabaseManager:
    ...
    def _seed_default_data(self, cursor):
        ...
        default_valves = [
            {"id": 10, "name": "1", "pin": "2", "enabled": True, "isOpen": False},
            {"id": 11, "name": "2", "pin": "3", "enabled": True, "isOpen": False},
            {"id": 12, "name": "3", "pin": "4", "enabled": True, "isOpen": False},
            {"id": 13, "name": "4", "pin": "5", "enabled": True, "isOpen": False},
            {"id": 14, "name": "5", "pin": "6", "enabled": True, "isOpen": False},
            {"id": 15, "name": "6", "pin": "7", "enabled": True, "isOpen": False},
            {"id": 16, "name": "7", "pin": "8", "enabled": True, "isOpen": False},
            {"id": 17, "name": "8", "pin": "9", "enabled": True, "isOpen": False},
            {"id": 18, "name": "9", "pin": "10", "enabled": True, "isOpen": False}
        ]
        cursor.execute("INSERT OR REPLACE INTO system_state (key, value) VALUES (?, ?)", ("valves", json.dumps(default_valves)))

        default_sensors = [
            {"id": "SENS-IN", "name": "Giriş Lazeri", "type": "INPUT", "pin": "17", "enabled": True, "device": "RASPI"},
            {"id": "SENS-OUT", "name": "Çıkış Lazeri", "type": "OUTPUT", "pin": "27", "enabled": True, "device": "RASPI"}
        ]
        cursor.execute("INSERT OR REPLACE INTO system_state (key, value) VALUES (?, ?)", ("sensors", json.dumps(default_sensors)))

        # Varsayılan Kapılar
        cursor.execute("INSERT OR REPLACE INTO system_state (key, value) VALUES (?, ?)", ("inputGate", json.dumps({"isOpen": False, "pin": "18"})))
        cursor.execute("INSERT OR REPLACE INTO system_state (key, value) VALUES (?, ?)", ("outputGate", json.dumps({"isOpen": False, "pin": "23"})))

        # Varsayılan Reçete
        cursor.execute("INSERT OR REPLACE INTO recipes (id, name, targetCount, fillTimeMs, waitAfterFillMs, active) VALUES (?, ?, ?, ?, ?, ?)",
                       ("RECIPE-1", "Standart Dolum", 9, 1500, 500, True))
